...
.service('$avg', function ($http) {
  ...
  _this.fetch = function fetch () {
    return $http.jsonp('http://localhost:8080/data/aggregates/fft?callback=JSON_CALLBACK')
  ...
  _this.visitors = function visitors () {
    return $http.jsonp('http://localhost:8080/data/visitors?callback=JSON_CALLBACK')
    .then(function (response) {
...
.service('$spacebrew', function ($timeout, $log, $http, muses, $location, $window) {
  ...
  _this.options = {
    name: subscriber,
    server : '127.0.0.1',//'208.66.31.59',
    description : 'Main dashboard in room',
    cloudbrain: '127.0.0.1'
  };
  _this.waves = [
    'alpha_absolute',
    'beta_absolute',
